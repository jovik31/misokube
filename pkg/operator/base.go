...
// AddDefaultInformerHandlers attaches default handlers that enqueue with the given source and events.
func (c *BaseOperator) AddDefaultInformerHandlers(inf cache.SharedIndexInformer, src Source, addEv, updEv, delEv Event) {
...
func (c *BaseOperator) worker(ctx context.Context, r EventReconciler) {
	...
	for {
		...
		// Ensure Done is always called before (re)queue decisions
		func(it WorkItem) {
			defer c.queue.Done(it)

			if err := r.ReconcileEvent(ctx, it.Source, it.Event, it.Resource); err != nil {
				c.logger.Error(err, "reconcile failed", "source", it.Source, "event", it.Event, "resource", it.Resource, "reconciler", r.Name())
