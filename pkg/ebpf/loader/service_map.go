...
func encodeServices(
	...
	desired := make(map[serviceFrontendKey]struct{}, len(services))
...
func encodeService(service Service) (encodedService, error) {
	...
	if len(service.Backends) > int(serviceBackendMaxEntries) {
		return encodedService{}, fmt.Errorf("too many Service backends")
	}

	backends := make([]serviceBackendValue, 0, len(service.Backends))
...
func encodeServiceKey(key ServiceKey) (serviceFrontendKey, error) {
	ip := key.IP.To4()
	if ip == nil {
	...
	if key.Port == 0 {
		return serviceFrontendKey{}, fmt.Errorf("Service port is zero")
	}
	if key.Protocol == 0 {
		return serviceFrontendKey{}, fmt.Errorf("Service protocol is zero")
	}

	var out serviceFrontendKey
	copy(out.Address[:], ip)
	binary.BigEndian.PutUint16(out.Port[:], key.Port)
...
func encodeServiceBackend(
	...
	if backend.Port == 0 {
		return serviceBackendValue{}, fmt.Errorf("Service backend port is zero")
	}
	...
	copy(out.Address[:], ip)
	binary.BigEndian.PutUint16(out.Port[:], backend.Port)
	if backend.ManagedPod {
...
func (m *ServiceMap) upsertServiceLocked(service encodedService) error {
	oldCount, err := m.frontendBackendCountLocked(service.key)
	if err != nil {
		return err
	}
	...
	for slot, backend := range service.backends {
		...
		if err := m.backend.Update(key, backend, ebpf.UpdateAny); err != nil {
			return fmt.Errorf(
	...
	if err := m.frontend.Update(
		...
		return fmt.Errorf("write Service frontend: %w", err)
	}
	...
	for slot := service.value.BackendCount; slot < oldCount; slot++ {
		...
		if err := deleteMapKey(m.backend, key); err != nil {
			return fmt.Errorf(
...
func (m *ServiceMap) deleteStaleFrontendsLocked(
	...
	if err := it.Err(); err != nil {
		return fmt.Errorf("iterate Service frontends: %w", err)
	}

	for _, key := range stale {
		if err := deleteMapKey(m.frontend, key); err != nil {
			return fmt.Errorf("delete stale Service frontend: %w", err)
		}
	}
	return nil
}
...
func (m *ServiceMap) deleteOrphanBackendsLocked() error {
	frontendCounts := make(map[serviceFrontendKey]uint32)

	frontendIt := m.frontend.Iterate()
	var frontendKey serviceFrontendKey
	var frontendValue serviceFrontendValue
	for frontendIt.Next(&frontendKey, &frontendValue) {
		frontendCounts[frontendKey] = frontendValue.BackendCount
	}
	if err := frontendIt.Err(); err != nil {
		return fmt.Errorf("iterate Service frontends for backend cleanup: %w", err)
	}
	...
	for backendIt.Next(&backendKey, &backendValue) {
		count := frontendCounts[backendKey.Frontend]
		if backendKey.Slot >= count {
			orphaned = append(orphaned, backendKey)
		}
	}
	if err := backendIt.Err(); err != nil {
		return fmt.Errorf("iterate Service backends: %w", err)
	}

	for _, key := range orphaned {
		if err := deleteMapKey(m.backend, key); err != nil {
			return fmt.Errorf("delete orphan Service backend: %w", err)
		}
	}
	return nil
}
...
func (m *ServiceMap) frontendBackendCountLocked(
	...
	var value serviceFrontendValue
	if err := m.frontend.Lookup(key, &value); err != nil {
		if errors.Is(err, ebpf.ErrKeyNotExist) {
			return 0, nil
		}
		return 0, fmt.Errorf("lookup Service frontend: %w", err)
	}
...
func openOrCreatePinnedServiceMap(
	...
	if err := os.MkdirAll(servicePinRoot, 0755); err != nil {
	...
	if !errors.Is(err, os.ErrNotExist) && !errors.Is(err, unix.ENOENT) {
		return nil, fmt.Errorf("open pinned Service map %s: %w", path, err)
	}

	m, err = ebpf.NewMap(spec)
	if err != nil {
		return nil, fmt.Errorf("create Service map %s: %w", path, err)
	}
	...
	if err := m.Pin(path); err == nil {
		...
		m.Close()
		return nil, fmt.Errorf("pin Service map %s: %w", path, err)
	}

	m.Close()
	m, err = ebpf.LoadPinnedMap(path, nil)
	if err != nil {
		return nil, fmt.Errorf("reopen Service map %s: %w", path, err)
	}
	if err := spec.Compatible(m); err != nil {
		m.Close()
		return nil, fmt.Errorf(
