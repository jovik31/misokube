...
func NewTCFirewall(ifaceName string) (*TCFirewall, error) {
	...
	const ifaceTenantMapName = "tc_iface_tenant"
	ifaceTenantMapPath := filepath.Join(pinRoot, ifaceTenantMapName)
	if pinnedMap, err := ebpf.LoadPinnedMap(ifaceTenantMapPath, nil); err == nil {
		fmt.Printf("Reusing pinned map %s\n", ifaceTenantMapPath)
		opts.MapReplacements = map[string]*ebpf.Map{
			ifaceTenantMapName: pinnedMap,
		}
	} else if !errors.Is(err, os.ErrNotExist) {
		return nil, fmt.Errorf("open pinned map %s: %w", ifaceTenantMapPath, err)
	} else {
		fmt.Printf("Pinned map %s not found; first load will create and pin it\n", ifaceTenantMapPath)
	}

	//fmt.Printf("Loading TC firewall BPF objects (pinning under %s)\n", opts.Maps.PinPath)

	if err := loadTcFirewallObjects(&objs, opts); err != nil {
		fmt.Printf("loadTcFirewallObjects failed (pinPath=%s): %v\n", opts.Maps.PinPath, err)
		return nil, fmt.Errorf("load TC objects: %w", err)
	}
	if objs.TcIfaceTenant != nil {
		fmt.Printf("TC firewall loaded with tc_iface_tenant map\n")
	}
...
func (fw *TCFirewall) AddInterfaceTenantBinding(ifindex int, tenant string) error {
	...
	if fw.objs.TcIfaceTenant == nil {
		return fmt.Errorf("tc_iface_tenant map not available")
	}
	// Ensure we always fit into the 64-byte fixed-size value.
	if len(tenant) > 63 {
		tenant = tenant[:63]
	}
	var value [64]byte
	copy(value[:], tenant)
	key := uint32(ifindex)
	return fw.objs.TcIfaceTenant.Update(key, value, ebpf.UpdateAny)
}
