...
func TestSetupVethReturnsHostIfIndex(t *testing.T) {
	...
	var tsoDisabledOn []string
	disableTSO = func(name string) error {
		tsoDisabledOn = append(tsoDisabledOn, name)
		return nil
	}
	...
	if len(tsoDisabledOn) != 0 {
		t.Fatalf("TSO unexpectedly disabled on %v", tsoDisabledOn)
	}
}
