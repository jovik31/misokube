...
func (n *Linux) EnsureVXLAN(config VXLANConfig) (VXLANLink, error) {
	...
	// LinkAdd may leave the netlink object with the provisional MAC generated
	// in userspace. Re-read the link after it is up so callers publish the MAC
	// actually assigned by the kernel.
	link, err = linkByName(config.Name)
	if err != nil {
		return VXLANLink{}, fmt.Errorf("refresh VXLAN %q after bring-up: %w", config.Name, err)
	}
