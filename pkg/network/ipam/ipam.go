...
func NewIPAM(subnet *net.IPNet) (*IPAM, error) {
	...
	if bits != 32 {
		return nil, fmt.Errorf("only IPv4 supported (got mask bits=%d)", bits)
	...
	if total < 4 {              // need more than 4 addresses to be useful
		return nil, fmt.Errorf("subnet too small for IPAM (got %s)", subnet.String())
	}
	...
	if podIPs <= 0 {
		return nil, fmt.Errorf("no usable IPs left after reserving bridge in subnet %s", subnet.String())
	}
...
func (ipam *IPAM) Capacity() int {
	...
	usable := total - 2 // exclude network + broadcast
	if usable <= 1 {
		return 0 // nothing left after reserving bridge
	}
	return usable - 1 // exclude bridge (network+1)
}

// remaining returns free ip slots
func (ipam *IPAM) Remaining() int {
...
func (ipam *IPAM) AllocateIP(podname string, id string, ifName string, netns string) (net.IP, error) {

	capacity := ipam.Capacity()
	if capacity <= 0 {
		return nil, fmt.Errorf("no free IPs available in subnet %s", ipam.Network)
	}
	...
	for idx, word := range ipam.bitmap {
		...
		ipam.IPs[podname] = &ContainerNetInfo{
			ID:     id,
			IFname: ifName,
			NetNS:  netns,
			IP:     ip,
...
func (ipam *IPAM) Free(ip net.IP) error {
	idx := ipam.ipToIndex(ip)
	if idx < 0 {
		return fmt.Errorf("ip %s not a pod IP in this subnet", ip)
	}
...
func (ipam *IPAM) indexToIP(idx int) net.IP {
...
func (ipam *IPAM) ipToIndex(ip net.IP) int {
	if ipam.Network == nil {
		return -1
	}
	ip4 := ip.To4()
