...
func (c *Controller) reconcileAssignments(ctx context.Context, tenant string, zones int) (assignmentResult, error) {
	...
	if len(assigned) < desired {
		...
		for _, node := range candidates[:need] {
			if err := c.patchNodeTenantLabel(ctx, node.Name, labelKey, true); err != nil {
				return assignmentResult{}, err
			}
...
func scaleDownCandidates(nodes []*corev1.Node, activePodNodes map[string]struct{}) []*corev1.Node {
...
func (c *Controller) patchNodeTenantLabel(ctx context.Context, nodeName, labelKey string, present bool) error {
	...
	if _, err := c.kube.CoreV1().Nodes().Patch(
		ctx,
		nodeName,
		...
		return fmt.Errorf("patch tenant label on node %s: %w", nodeName, err)
	}
	return nil
}

func nodeEligible(node *corev1.Node) bool {
	return node != nil && !node.Spec.Unschedulable && nodeReady(node)
}
