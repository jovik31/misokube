...
func TestHandleAddUsesPodMetadata(t *testing.T) {
	...
	if podNetwork.addReq != want {
		t.Fatalf("got request %+v, want %+v", podNetwork.addReq, want)
	}
...
func TestHandleDelDoesNotNeedPodMetadata(t *testing.T) {
	podNetwork := &fakePodNetwork{}
	server := newTestServer(t, newPodLister(t), podNetwork)
	...
	if podNetwork.delCalls != 1 {
		t.Fatalf("got %d DelPod calls, want 1", podNetwork.delCalls)
	}
	...
	if podNetwork.delReq != want {
		t.Fatalf("got request %+v, want %+v", podNetwork.delReq, want)
	}
}
...
func TestHandleCheck(t *testing.T) {
	...
	if podNetwork.checkCalls != 1 {
		t.Fatalf("got %d CheckPod calls, want 1", podNetwork.checkCalls)
	}
}
...
func newPodLister(t *testing.T, pods ...*corev1.Pod) corev1listers.PodLister {
	t.Helper()
